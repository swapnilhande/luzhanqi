...
public class MoveGenerator {
	 ...
	 * @param start
	 * @return returns array of integer of length 2.
	 * first is source index and second is target 
	 */
	...
	/**
	 * This method evaluates all possible moves
	 * and updates bestMove with best evaluation
	 */
	public void getBestMove(long startTime){
        ...
        for (int i = 0; i < player.getCanMove().length; i++) {
            if (player.getBoard()[i].getOwner() == player.getMyNumber()) {
                ...
                if(!isTimeLeft(startTime)) return;
            }
        }
        if (noMove()) return;
	...
	/**
	 * This method determines if time to determine
	 * is left or not
	 * @param startTime
	 * @return true if time left
	 * false otherwise
	 */
	...
	/**
	 * This method determines if the bestMove is updated
	 * after all possible moves
	 * @return true if bestMove is updated 
	 * false otherwise
	 */
	 ...
	 * @param valuation
	 */
	public void updateBestMoveInDirection(int source, int direction, double valuation){
	...
	private void updateBestMove(int source, int target) {
	 ...
	 /**
     * This method update the three turn parameters
     * for the move 
    ...
    public double evaluateMove(int from, int to) {
        ...
        if (hisPiece > Constants.PIECE_UNKNOWN && myPiece > hisPiece) {
            ...
            valuation += totalValue * player.getPieceParams()[myPiece][Constants.BEAT_OPPONENT];
        }
        ...
        if (player.getBoard()[58].getPiece() == Constants.PIECE_FLAG) {
            ...
            valuation += (15 - distance); // + its weight
        ...
        // Positions closer to headquarters get more weights

        // 6.1 Instead give more weights if to is an enemy safe position
        ...
        if (player.getBoard()[from].getPiece() == Constants.PIECE_ENGINEER &&
                ...
                && to > 49) {
            valuation += 0; // add weight to this move
        }
        ...
        for (int i = 0; i < 15; i++) {
            if (player.getBoard()[i].getOwner() == player.getHisNumber()) {
                ...
                if (to == i && player.getBoard()[from].getPiece() > player.getBoard()[i].getPiece()) {
        ...
        // weight
        if (!player.getHasMoved()[to] && player.getBoard()[to].getPiece() == Constants.PIECE_UNKNOWN
                && to > 49) {
            valuation += (Constants.PIECE_FIELDMARSHAL - player.getBoard()[from]
                    .getPiece()) * 0; // add weights
        }
